...
int main(int argc, char ** argv){
	...
	TCut inclusive = "(lV > 15 && lW > 15 && p_e < Ebeam && E_tot > 0.25 && e_vtz < 10 && e_vtz > -15 && E_tot/p_e < 0.3 && E_tot/p_e > 0.15 && p_e>2 && nMult == 1)";
	...
	c0 -> Modified();
	c0 -> Update(); 
	c0 -> Print(Form("compare_tagged.pdf(" ));
	c_p_e	->Print(Form("compare_tagged.pdf(" ));
	c_p_comp->Print(Form("compare_tagged.pdf(" ));
	c_th_e	->Print(Form("compare_tagged.pdf(" ));
	c_ph_e	->Print(Form("compare_tagged.pdf(" ));
	c_q	->Print(Form("compare_tagged.pdf(" ));
	c_th_q	->Print(Form("compare_tagged.pdf(" ));
	c_ph_q	->Print(Form("compare_tagged.pdf(" ));
	c_nu	->Print(Form("compare_tagged.pdf(" ));
	c_Q2	->Print(Form("compare_tagged.pdf(" ));
	c_xB	->Print(Form("compare_tagged.pdf(" ));
	c_W	->Print(Form("compare_tagged.pdf(" ));
	c0 -> Print(Form("compare_tagged.pdf)" ));
...
TTree* readTree(TFile* inFile){
	...
	inTree->Branch("nMult"		,&nMult			);
	inTree->Branch("barID"		,&barID			,"barID[nMult]/D"	);
	inTree->Branch("dL_n"		,&dL_n			,"dL_n[nMult]/D"	);
	inTree->Branch("theta_n"	,&theta_n		,"theta_n[nMult]/D"	);
	inTree->Branch("phi_n"		,&phi_n			,"phi_n[nMult]/D"	);
	inTree->Branch("p_n"		,&p_n			,"p_n[nMult]/D"		);
	inTree->Branch("nTime"		,&nTime			,"nTime[nMult]/D"	);
	inTree->Branch("nEdep"		,&nEdep			,"nEdep[nMult]/D"	);
