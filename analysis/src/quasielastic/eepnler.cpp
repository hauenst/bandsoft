...
bool checkNeutron(int nHits, vector<int> barKey, vector<int> layer, vector<double> meanADC, vector<double> meanTimeFADC, 
			vector<TVector3> hitPos, double starttime, double* FADC_GLOB_SHIFT);
...
int main(int argc, char** argv) {
	...
	for( int i = 2 ; i < argc ; i++ ){
		...
		while(reader.next()==true){
			...
			bool nPass = checkNeutron(nHits, barKey, layer, meanADC, meanTimeFADC, hitPos, starttime, FADC_GLOB_SHIFT);
...
bool checkNeutron(int nHits, vector<int> barKey, vector<int> layer, vector<double> meanADC, vector<double> meanTimeFADC, vector<TVector3> hitPos, double starttime, double* FADC_GLOB_SHIFT) {
	...
	for (int hit = 0; hit < nHits; hit++) {
		...
		// cut on veto bars
		if (layer[hit] == 6) return false;

		// cut on time of flight
		double ToF = meanTimeFADC[hit] - starttime - FADC_GLOB_SHIFT[barKey[hit]];
		if (ToF < 20 || ToF > 50) return false;	

		// cut on min E_dep
		// currently assuming 2000 ch/MeV
		if (meanADC[hit] < 10000) return false;
