...
int main(int argc, char** argv) {
	...
	c0 -> Print(Form("corr2ndIter_tdc_%s(" ,outputName.Data()));
	c1 -> Print(Form("corr2ndIter_ftdc_%s(",outputName.Data()));
	...
	for(int is = 0 ; is < 5 ; is++){
		...
		for(int il = 0 ; il < 6 ; il++){
			...
			cSLC_tdc[is][il] -> Print(Form("corr2ndIter_tdc_%s",outputName.Data()));
			cSLC_ftdc[is][il] -> Modified();     
			cSLC_ftdc[is][il] -> Update();
			cSLC_ftdc[is][il] -> Print(Form("corr2ndIter_ftdc_%s",outputName.Data()));
		}
	}
	c0 -> Print(Form("corr2ndIter_tdc_%s)",outputName.Data()));
	c1 -> Print(Form("corr2ndIter_ftdc_%s)",outputName.Data()));
	...
	ofstream tabL, tabR;
	tabL.open("corr2ndIter_effective_velocity.txt");
	tabR.open("corr2ndIter_lr_offsets.txt");
