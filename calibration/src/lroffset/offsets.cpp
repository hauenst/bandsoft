...
int main(int argc, char** argv) {
	...
	c0 -> Print("tdc_results_offsets.pdf(");
	c1 -> Print("ftdc_results_offsets.pdf(");
	...
	for(int is = 0 ; is < 5 ; is++){
		...
		for(int il = 0 ; il < 6 ; il++){
			...
			cSLC_tdc[is][il] -> Print("tdc_results_offsets.pdf");
			cSLC_ftdc[is][il] -> Modified();     
			cSLC_ftdc[is][il] -> Update();
			cSLC_ftdc[is][il] -> Print("ftdc_results_offsets.pdf");
		}
	}
	c0 -> Print("tdc_results_offsets.pdf)");
	c1 -> Print("ftdc_results_offsets.pdf)");
