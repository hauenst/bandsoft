...
void walkCorr(	std::vector<double> *adcs		,
		...
		double widthCut				,
		TH2F * hist				);
void fitTW(TH2F * hist , TCanvas * c, int cd, int s, int l, int co, int o, double cut, double &par1, double &par2 , double &par3,
			double &par1_err, double &par2_err, double &par3_err );
...
int main(int argc, char** argv) {
	...
	for(int i = 0 ; i < nHistos ; i++){
		...
		h2_tdc_amp_L[i] = new TH2F(Form("h2_tdc_amp_L_%i",i),";AMP_{L};t_{TDC,L} - ref [ns]",600,0,4200,3000,0,300);
		h2_tdc_amp_R[i] = new TH2F(Form("h2_tdc_amp_R_%i",i),";AMP_{R};t_{TDC,R} - ref [ns]",600,0,4200,3000,0,300);
		...
		for(int il = 0 ; il < 6 ; il++){
			...
			for(int cIdx = 0 ; cIdx < slc[il][is] ; cIdx++){
				...
				if( h2_tdc_adc_L[identifier]->Integral()  ){
					double par1 = 0, par2 = 0, par3 = 0, par1e = 0, par2e = 0, par3e = 0;
					...
					fitTW( h2_tdc_adc_L[identifier] , cSLC_adc[is][il] ,  4*cIdx+2 , (is+1), (il+1), (cIdx+1), 
							0, 4000, par1 , par2 , par3, par1e, par2e , par3e );
					// Fit results:
					par1_L[identifier][0] = par1;
					par2_L[identifier][0] = par2;
					par3_L[identifier][0] = par3;
					par1e_L[identifier][0] = par1e;
					par2e_L[identifier][0] = par2e;
					par3e_L[identifier][0] = par3e;
					par1 = 0, par2 = 0, par3 = 0, par1e = 0, par2e = 0, par3e = 0;

					fitTW( h2_tdc_amp_L[identifier] , cSLC_amp[is][il] ,  4*cIdx+2 , (is+1), (il+1), (cIdx+1), 
							0, 300, par1 , par2 , par3, par1e, par2e , par3e );
					par1_L[identifier][1] = par1;
					par2_L[identifier][1] = par2;
					par3_L[identifier][1] = par3;
					par1e_L[identifier][1] = par1e;
					par2e_L[identifier][1] = par2e;
					par3e_L[identifier][1] = par3e;
				}
				if( h2_tdc_adc_R[identifier]->Integral()  ){
					...
					if( identifier!= 416 ){
						fitTW( h2_tdc_adc_R[identifier] , cSLC_adc[is][il] ,  4*cIdx+4 , (is+1), (il+1), (cIdx+1), 
							1, 4000,par1 , par2 , par3, par1e, par2e , par3e );
					}
					par1_R[identifier][0] = par1;
					par2_R[identifier][0] = par2;
					par3_R[identifier][0] = par3;
					par1e_R[identifier][0] = par1e;
					par2e_R[identifier][0] = par2e;
					par3e_R[identifier][0] = par3e;
					par1 = 0, par2 = 0, par3 = 0, par1e = 0, par2e = 0, par3e = 0;

					// Fit results:
					if( identifier!= 416 ){
						fitTW( h2_tdc_amp_R[identifier] , cSLC_amp[is][il] ,  4*cIdx+4 , (is+1), (il+1), (cIdx+1), 
							1, 300,par1 , par2 , par3, par1e, par2e , par3e );
					}
					par1_R[identifier][1] = par1;
					par2_R[identifier][1] = par2;
					par3_R[identifier][1] = par3;
					par1e_R[identifier][1] = par1e;
					par2e_R[identifier][1] = par2e;
					par3e_R[identifier][1] = par3e;
				}
				/*
	...
	ofstream tabL_adc, tabR_adc, tabL_amp, tabR_amp ;
	tabL_adc.open("firstIter_timeWalkPar_adc_L.txt");
	tabR_adc.open("firstIter_timeWalkPar_adc_R.txt");
	tabL_amp.open("firstIter_timeWalkPar_amp_L.txt");
	tabR_amp.open("firstIter_timeWalkPar_amp_R.txt");
...
void walkCorr(	std::vector<double> *adcs		,
	...
	while( currBin < maxBin ){
		...
		int step = doProj( hist , currBin , false , 0, xPt, yPt, yEr, ySig, ySigEr , 500, maxBin );
		currBin += step ;
